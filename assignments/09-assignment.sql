...
-- Hint: You will have to use ROUND() and CAST() to convert the variable to numeric.SELECT 
SELECT
    team, 
    ROUND(CAST(AVG(points) AS NUMERIC), 2) AS average_points
FROM 
    drivers
GROUP BY 
    team;
...
SELECT *
FROM drivers
WHERE points >= (SELECT percentile_80 FROM Percentile);
